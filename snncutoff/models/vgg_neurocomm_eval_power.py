...
class wirelss_ch_odfm(nn.Module):
    def __init__(self,E,
        ...
        self.group_size = 8
        self.num_path = 5
        self.E = E  # average energy consumption of per data subcarrier, in dB
    ...
    def _forward_analog(self, input):
        ...
        for t in range(input.size(1)):
            ...
            E_watt = E_t[t]
            # E_watt = 10 ** (self.E / 10)  # transform db to Watt. Make E_watt to be dynamic for different time t
    ...
    def __generate_mbit_pam(self, m, device):
        if m == 0:
            # For m = 0, we only need two points: 0 and 1
            constellation_points = torch.tensor([0, 1], dtype=torch.float32, device=device)
        elif m == 1:
            ...
            # For m >= 2, we need 2^m points with zero at the first index
            num_points = 2 ** m  # Total number of constellation points
            
            # Generate (num_points - 1) points symmetrically spaced around zero
            step_size = 2.0 / (num_points - 1)  # Step size between points

            # Generate positive points and reflect for symmetric negative points
            positive_points = torch.linspace(step_size, 1.0, (num_points // 2), dtype=torch.float32, device=device)
            negative_points = -positive_points  # Symmetric negative points

            # Concatenate zero, negative, and positive points, with zero as the first element
            constellation_points = torch.cat((
                torch.tensor([0.0], device=device),  # First element is zero
                negative_points.flip(0),  # Negative points (in descending order)
                positive_points
            ))

            # Exclude zero and calculate the mean squared value of non-zero points
            non_zero_points = constellation_points[constellation_points != 0]
            mean_squared_value = torch.mean(non_zero_points ** 2)
            
            # Scale the constellation so that the mean squared value of non-zero points is 1
            scaling_factor = torch.sqrt(1 / mean_squared_value)
            scaled_non_zero_points = non_zero_points * scaling_factor

            # Reinsert zero as the first element
            constellation_points = torch.cat((
                torch.tensor([0.0], device=device),  # Reinsert zero at the first index
                scaled_non_zero_points
            ))
