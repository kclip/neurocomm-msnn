...
class wirelss_ch_odfm(nn.Module):
    def __init__(self,E,
        ...
        self.power = 'fixed'
        self.decay_rate = 0.9  # Decay rate we have tested 0.7 
    ...
    def _forward_digital(self, input):
        ...
        if self.power == 'foxed':
            E_t =  np.ones(input.size(0))*E_max
        elif self.power == 'decay':
            E_t = E_max * (1 - self.decay_rate  * np.arange(1, input.size(0) + 1) / input.size(0))
            E_t = E_t * (E_max * input.size(0) / np.sum(E_t))  # Normalize to satisfy total energy constraint
        elif self.power == 'increase':
            E_t = E_max * (1 - self.decay_rate  * np.arange(1, input.size(0) + 1) / input.size(0))
            E_t = E_t * (E_max * input.size(0) / np.sum(E_t))  # Normalize to satisfy total energy constraint
            E_t = E_t[::-1]
