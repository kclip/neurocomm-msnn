...
class wirelss_ch_odfm(nn.Module):
    def __init__(self,E,
        ...
        self.rec_rate = []
        self.power_constrs = 'peak'
        # self.block_power_constrs = True
        self.data = np.load('./channel.npz')['arr_0']


    def sample_from_data_tf(self, shape):
        # Sample values from your custom data
        data = tf.convert_to_tensor(self.data, dtype=tf.complex64)
        indices = tf.random.uniform(shape=(shape[0],), minval=0, maxval=data.shape[0], dtype=tf.int32)
        data = tf.gather(data, indices, axis=0)
        indices = tf.random.uniform(shape=(shape[1],), minval=0, maxval=data.shape[1], dtype=tf.int32)
        data = tf.gather(data, indices, axis=1)
        return data

    def sample_from_data(self, shape):
        # Sample values from your custom data
        data = torch.tensor(self.data, dtype=torch.complex64).to('cuda')
        # indices = torch.randint(shape=(shape[0],), min=0, max=data.shape[0], dtype=torch.int32)
        indices = torch.randint(0, data.shape[0], size=(shape[0],), dtype=torch.long, device='cuda')
        data = data[indices]
        indices = torch.randint(0, data.shape[1], size=(shape[1],), dtype=torch.long, device='cuda')
        data = data[:,indices]
        return data
    ...
    def _forward_digital(self, input):
        ...
        for t in range(input.size(0)):
            ...
            OFDM_data = tf.tensor_scatter_nd_update(OFDM_data, indices, values)
            # print(OFDM_data)
            # channel_power = (1/(2*self.num_path))**0.5
            # channelResponse = tf.complex(tf.random.normal((batch_size, max_num_ofdm, self.num_path), 0.0, channel_power), tf.random.normal((batch_size, max_num_ofdm, self.num_path), 0.0, channel_power))
            ...
            H_exact = self.sample_from_data_tf(list(OFDM_data.shape))
            ...
            equalized_data = equalized_data/tf.cast(tf.reduce_max(tf.abs(equalized_data),axis=[-2,-1],keepdims=True),dtype=tf.complex64) # normalization
        ...
        # For checking purpose
        # rec_rate =  (input == ((input_decoding_snn+1)/(2**self.num_bit))).sum().item()/input.numel()
        # print("Bit recovery rate: ", (input == ((input_decoding_snn+1)/(2**self.num_bit))).sum().item()/input.numel())
        # self.rec_rate.append(rec_rate)
    ...
    def _forward_analog(self, input):
        ...
        temperature_en = 2  # You can adjust this value
        temperature_de = 1  # You can adjust this value
        # temperature = 1  # You can adjust this value
        ...
        for t in range(input.size(1)):
            ...
            E_watt = 10 ** (self.E / 10)  # transform db to Watt
            if self.training:
                binary_indices = torch.arange(2**self.num_bit+1, dtype=torch.float32,device=input.device).unsqueeze(0).unsqueeze(0) 
                logits = -torch.abs(input[:, t, :].unsqueeze(-1)  - binary_indices)
                soft_one_hot = F.softmax(logits / temperature_en, dim=-1)
                pam_values = torch.sum(soft_one_hot * pam_points.unsqueeze(0).unsqueeze(0), dim=-1) + 0.j
            else:
                pam_values = pam_points[input[:, t, :].long()] + 0.j  # (batch_size, num_neurons)

            # print('index:',input[:, t, :].long().max(),'real:',pam_values.real.max())
            if self.power_constrs == 'block':
            ...
            # print(torch.sum((pam_values*torch.sqrt(gamma)).abs() ** 2, dim=1)/num_data_carriers)
            # # print(E_watt)
            ...
            # max_num_ofdm = self.num_ofdma
            # H_exact = self._cdl_channel(max_num_ofdm,OFDM_data.size(0),OFDM_data.size(-1))
            # H_exact = tf.reshape(H_exact, [H_exact.shape[0], H_exact.shape[-2], H_exact.shape[-1]])
            # H_exact = torch.from_numpy(H_exact.numpy()).long().to(input.device)
            H_exact = self.sample_from_data(list(OFDM_data.shape))
            noise_power = 1e-3
            ...
            distances = torch.abs(mrc_output.unsqueeze(-1) - pam_points.unsqueeze(0).unsqueeze(0))  # (batch_size, num_data_carriers, 2^num_bits+1)
            if self.training:
                soft_probs = torch.softmax(-distances / temperature_de, dim=-1)  # Shape: (batch_size, num_data_carriers, 2^num_bits+1)
                symbol_indices = torch.arange(2 ** self.num_bit+1, device=distances.device).unsqueeze(0).unsqueeze(0)  # Shape: (batch_size, num_data_carriers, 2^num_bits+1)
        ...
        # print(rec_rate/(t+1))
        # save_pickle(self.rec_rate,name='E_'+str(self.E)+'G'+str(self.num_bit)+'_'+self.modulation, path=os.path.dirname('/LOCAL2/dengyu/project/neurocomm/snr_false_rate/'))
        # print ("Recovery rate: ", rec_rate/(t+1))
...
class VGG_NeuroComm_emu(nn.Module):
    def __init__(self,num_classes = 11,E=1,
                 neuron_params: dict = {'vthr': 1.0},):
        super(VGG_NeuroComm_emu, self).__init__()
        # pool = nn.MaxPool2d(2)
        # self.features = nn.Sequential(
        #     Conv2dLayer(2,64,4,4,padding='valid'),
        #     Conv2dLayer(64,128,3,1,1),
