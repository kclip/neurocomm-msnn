...
class wirelss_ch_odfm(nn.Module):
    ...
    def _forward_analog(self, input):
        ...
        pam_points = self._generate_mbit_pam(self.num_bit).to(input.device) 
        ...
        num_allcarriers = num_data_carriers + num_pilot
        allCarriers = torch.arange(num_allcarriers).to(input.device) 
        pilotCarriers = torch.arange(0, num_allcarriers, self.pilot_interval).to(input.device) 
        if pilotCarriers[-1] != allCarriers[-1]:
        ...
        for t in range(input.size(1)):
            OFDM_data = torch.zeros((batch_size, self.num_ofdma, num_allcarriers), dtype=torch.complex64).to(input.device) 
            OFDM_data[:, :, pilotCarriers] = self.pilotValue  # assign a known pilot symbol to all pilot subcarriers in an OFDM symbol

            # calculate the power scaling 
            E_watt = 10 ** (self.E / 10)  # transform db to Watt
            pam_values = pam_points[input[:, t, :].long()] + 0.j  # (batch_size, num_neurons)

            gamma = num_data_carriers * E_watt/torch.sum(pam_values ** 2, dim=1)  # (batch_size,)
            # print(E_watt)
            # print(gamma*torch.sum(input[:,t,:] ** 2, dim=1)/num_data_carriers)
            OFDM_data[:, :, dataCarriers] = (pam_values*torch.sqrt(gamma).unsqueeze(1)).unsqueeze(1).expand(-1, self.num_ofdma, -1).to(torch.complex64) # assign all the data symbols to their corresponding data subcarriers in an OFDM symbol

            channelResponse = (torch.normal(mean=0, std=math.sqrt(1/2), size=(batch_size, self.num_ofdma, self.num_path)) +1j * torch.normal(mean=0, std=math.sqrt(1/2), size=(batch_size, self.num_ofdma, self.num_path))).to(input.device)  # the impulse response of the wireless channel
            H_exact = torch.fft.fft(channelResponse, n=num_allcarriers, dim=2)  # (batch_size, num_ofdm, num_allcarriers)

            noise_power = 1e-3
            OFDM_demod = OFDM_data * H_exact + torch.normal(mean=0, std=noise_power, size=(batch_size, self.num_ofdma, num_allcarriers)).to(input.device)
            ...
            Hest_abs = torch.lerp(
                torch.abs(Hest_at_pilots[torch.arange(batch_size).unsqueeze(1).unsqueeze(2), torch.arange(self.num_ofdma).unsqueeze(0).unsqueeze(2), lower_indices]),
                torch.abs(Hest_at_pilots[torch.arange(batch_size).unsqueeze(1).unsqueeze(2), torch.arange(self.num_ofdma).unsqueeze(0).unsqueeze(2), upper_indices]),
            ...
            Hest_phase = torch.lerp(
                torch.angle(Hest_at_pilots[torch.arange(batch_size).unsqueeze(1).unsqueeze(2), torch.arange(self.num_ofdma).unsqueeze(0).unsqueeze(2), lower_indices]),
                torch.angle(Hest_at_pilots[torch.arange(batch_size).unsqueeze(1).unsqueeze(2), torch.arange(self.num_ofdma).unsqueeze(0).unsqueeze(2), upper_indices]),
                weights.unsqueeze(0).unsqueeze(0)  # Shape: (1, 1, num_data_carriers)
            )
            Hest = Hest_abs * torch.exp(1j * Hest_phase)  # (batch_size, num_ofdm, num_data_carriers)

            equalized_data = OFDM_demod[:, :, dataCarriers] / Hest /torch.sqrt(gamma).unsqueeze(1).unsqueeze(2) # equalized data symbols (batch_size, num_ofdm, num_data_carriers)
    ...
    def _generate_mbit_pam(self, m):
        if m == 1:
            constellation_points = torch.tensor([0, -1, 1], dtype=torch.float32)
        elif m == 2:
            constellation_points = torch.tensor([0, -1.3416, -0.4472, 0.4472, 1.3416], dtype=torch.float32)
        elif m == 3:
            constellation_points = torch.tensor([0, -1.755, -1.253, -0.752, -0.251, 0.251, 0.752, 1.253, 1.755], dtype=torch.float32)
        elif m == 4:
            constellation_points = torch.tensor([0, -1.627, -1.410, -1.193, -0.976, -0.759, -0.542, -0.325, -0.108, 0.108, 0.325, 0.542, 0.759, 0.976, 1.193, 1.410, 1.627], dtype=torch.float32)
        elif m == 5:
            constellation_points = torch.tensor([0, -1.679, -1.570, -1.462, -1.354, -1.246, -1.137, -1.029, -0.921, -0.812, -0.704, -0.596, -0.487, -0.379, -0.271, -0.162, -0.054, 0.054, 0.162, 0.271, 0.379, 0.487, 0.596, 0.704, 0.812, 0.921, 1.029, 1.137, 1.246, 1.354, 1.462, 1.570, 1.679], dtype=torch.float32)
        else:
            raise ValueError("Unsupported value of m: {}".format(m))
...
class VGG_NeuroComm(nn.Module):
    def __init__(self,num_classes = 11,E=1,
                 neuron_params: dict = {'vthr': 1.0},):
        super(VGG_NeuroComm, self).__init__()
        # pool = nn.MaxPool2d(2)
        # self.features = nn.Sequential(
        #     Conv2dLayer(2,64,4,4,padding='valid'),
        #     Conv2dLayer(64,128,3,1,1),
